...
- [ ] **データ永続化・キャッシュ機構の実装と強化**
    - [x] **データストア・永続化基盤の共通化**
        - ファイル入出力やキャッシュ管理を一元化するストレージユーティリティ (`src/common/storage.py`) の設計・実装 (完了)
        - 保存先ディレクトリ (`data/price/`, `data/financial/`, `data/cache/` 等) の自動作成・パス管理の統一 (完了)
    - [x] **OHLCV（株価・出来高データ）の Parquet 永続化**
        - `Data Collector` による株価・出来高データ取得時に、ローカルファイル（Parquet形式）へ効率的に保存・読み込みするロジックの実装 (完了)
        - 差分更新（既存データと新規取得データのマージ・重複排除）機能の実装 (完了)
    - [x] **財務データおよびメタデータの永続化**
        - IRBank等から取得したファンダメンタルズ・財務データの構造化保存（JSON または SQLite 形式）の実装 (完了)
        - 銘柄マスタ（銘柄コード、名称、セクター等）のキャッシュ・永続化機構の実装 (完了)
    - [x] **インジケータ・特徴量・パターンの計算結果キャッシュ機構の強化**
        - 各種計算結果を有効期限や日付ベースでキャッシュする機構の整備 (完了)
        - 再計算をスキップしてキャッシュから高速ロードできる仕組みの統合 (完了)
    - [x] **データ永続化・キャッシュの単体・結合テスト**
        - 各ストレージ・キャッシュ処理（保存、読み込み、差分マージ等）に対する単体テストの実装と検証 (完了)
- [ ] **高機能なスクリーニングとランキング**
    - [x] 複数の条件を組み合わせたスクリーニング機能の実装
    - [x] ランキング表示のカスタマイズ機能
    - [x] スクリーニングおよびランキング・プレゼンテーション（CLI出力）の単体テスト実装と検証完了
- [ ] **エラーハンドリングとロギングの強化**
    ...
    - [x] メイン実行フロー (`src/main.py`) の段階的検証と実装
        - [x] ステップ1: `src/main.py` をデータ取得（OHLCV・財務データ）のみの検証用に縮小・リファクタリング
        - [x] ステップ2: データ取得処理単体での正常動作確認（複数銘柄のループ処理検証）
        - [x] ステップ3: テクニカル指標計算（Indicator Calculator）の結合と単体検証
        - [x] ステップ4: 特徴量計算（Feature Engine）の結合と、特徴量が0件になる原因の特定・修正
            - [x] 4-1: 特徴量マネージャー（`FeatureManager`）および各特徴量クラスの単体テスト確認
            - [x] 4-2: 代表的な1つの特徴量（例: `M001: MAAlignmentScore`）を `main.py` に組み込んで検証
            - [x] 4-3: テクニカル系特徴量を順次追加
                - [x] 4-3-1: `P001_PullbackScore`（押し目スコア）の結合と検証
                - [x] 4-3-2: `P002_BreakoutScore`（ブレイクアウトスコア）の結合と検証
                - [x] 4-3-3: `T001_TrendStrengthScore`（トレンド強度スコア）の結合と検証
                - [x] 4-3-4: `V001_VolumeScore`（出来高スコア）の結合と検証
                - [x] 4-3-5: `O001_MomentumScore`（モメンタムスコア）の結合と検証
                - [x] 4-3-6: `R001_VolatilityScore`（ボラティリティスコア）の結合と検証
            - [x] 4-4: ファンダメンタルズ系特徴量を追加（F001〜F005）および財務データ連携の検証
        - [x] ステップ5: パターン検出・スコアリングの結合と単体検証
        - [x] ステップ6: ランキング・プレゼンテーション出力までの全パイプライン結合テスト完了
- [ ] **パフォーマンス最適化**
...
## 5. 実行ログ (最新の作業のみ記載)
- 2026/08/31: ステップ5（パターン検出・スコアリングの結合）およびステップ6（ランキング・プレゼンテーション出力までの全パイプライン結合テスト）を完了。全10銘柄を用いた一連の分析パイプライン（OHLCV・財務データ取得、指標計算、特徴量計算、パターン検出、スコアリング、ランキング順位付け、CLI出力）を正常に実行・検証。`docs/_plan.md` を更新。
- 2026/08/31: 全13種類の特徴量（テクニカル系・ファンダメンタルズ系すべて）を `main.py` のパイプラインに一括統合し、全10銘柄での計算およびキャッシュ保存が正常に完了することを確認（ステップ4完了）。`docs/_plan.md` を更新。
